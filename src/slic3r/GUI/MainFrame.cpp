...
#ifdef __APPLE__
class CoPrintSlicerTaskBarIcon : public wxTaskBarIcon
{
public:
    CoPrintSlicerTaskBarIcon(wxTaskBarIconType iconType = wxTBI_DEFAULT_TYPE) : wxTaskBarIcon(iconType) {}
...
#else // _WIN32
    return wxIcon(Slic3r::var("CoPrintSlicer_128px.png"), wxBITMAP_TYPE_PNG);
#endif // _WIN32
}
...
*/
    ...
    case GUI_App::EAppMode::Editor:
        m_taskbar_icon = std::make_unique<CoPrintSlicerTaskBarIcon>(wxTBI_DOCK);
        m_taskbar_icon->SetIcon(wxIcon(Slic3r::var("CoPrintSlicer-mac_256px.ico"), wxBITMAP_TYPE_ICO), "CoPrintSlicer");
        break;
...
void MainFrame::init_tabpanel() {
    ...
    // PrinterWebView (WebView2) is created lazily: from show_device() when needed, and via CallAfter
    // at the end of MainFrame construction so the top-level window hierarchy is ready (avoids startup crashes).
...
// See https://github.com/CoPrintSlicer/CoPrintSlicer/issues/8152
...
static wxMenu* generate_help_menu()
{
    ...
    // Report a bug
    //append_menu_item(helpMenu, wxID_ANY, _L("Report Bug(TODO)"), _L("Report a bug of CoPrintSlicer"),
    //    [](wxCommandEvent&) {
    //        //TODO
    //    });
    // Check New Version
...
void MainFrame::init_menubar_as_editor()
{
    ...
    //auto preference_item = new wxMenuItem(parent_menu, CoPrintSlicerMenuPreferences + bambu_studio_id_base, _L("Preferences") + "\t" + ctrl + ",", "");
#else
...
//    });
...
#ifdef __APPLE__
    wxString about_title = wxString::Format(_L("&About %s"), SLIC3R_APP_FULL_NAME);
    //auto about_item = new wxMenuItem(parent_menu, CoPrintSlicerMenuAbout + bambu_studio_id_base, about_title, "");
        //parent_menu->Bind(wxEVT_MENU, [this, bambu_studio_id_base](wxEvent& event) {
        //    switch (event.GetId() - bambu_studio_id_base) {
        //        case CoPrintSlicerMenuAbout:
        //            Slic3r::GUI::about();
        //            break;
        //        case CoPrintSlicerMenuPreferences:
        //            CallAfter([this] {
        //                PreferencesDialog dlg(this);
        //                dlg.ShowModal();
        //#if ENABLE_GCODE_LINES_ID_IN_H_SLIDER
    ...
    append_menu_item(m_topbar->GetCalibMenu(), wxID_ANY, _L("Tutorial"), _L("Calibration help"),
        [this](wxCommandEvent&) {
            std::string url = "https://www.coprintslicer.com/wiki/Calibration";
            if (const std::string country_code = wxGetApp().app_config->get_country_code(); country_code == "CN") {
                // Use gitee mirror for China users
                url = "https://gitee.com/n0isyfox/coprintslicer-docs/wikis/%E6%A0%A1%E5%87%86/%E6%89%93%E5%8D%B0%E5%8F%82%E6%95%B0%E6%A0%A1%E5%87%86";
            }
    ...
    append_menu_item(calib_menu, wxID_ANY, _L("Tutorial"), _L("Calibration help"),
        [this](wxCommandEvent&) { wxLaunchDefaultBrowser("https://www.coprintslicer.com/wiki/Calibration", wxBROWSER_NEW_WINDOW); }, "", nullptr,
        [this]() {return m_plater->is_view3D_shown();; }, this);
...
{
    BOOST_LOG_TRIVIAL(trace) << "load_printer_url skipped (device tab detached from print host):" << url;
    (void) apikey;
    return;
}
