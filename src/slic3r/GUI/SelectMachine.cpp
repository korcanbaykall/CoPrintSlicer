    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    m_link_edit_nozzle->SetForegroundColour("#E6A800");
    ...
    m_statictext_finish->SetForegroundColour(wxColour(230, 168, 0));
...
void PrintOptionItem::doRender(wxDC& dc)
{
    ...
    for (auto it = m_ops.begin(); it != m_ops.end(); ++it)
    {
        ...
        {
            const wxColour& clr = m_enable ? StateColor::darkModeColorFor("#E6A800") : StateColor::darkModeColorFor(wxColour(144, 144, 144));
            dc.SetPen(wxPen(clr));
