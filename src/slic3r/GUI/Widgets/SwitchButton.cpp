...
SwitchButton::SwitchButton(wxWindow* parent, wxWindowID id)
    ...
    , thumb_color(std::pair{0xE6A800, (int) StateColor::Checked}, std::pair{0xD9D9D9, (int) StateColor::Normal})  // CoPrintSlicer yellow
{
...
void SwitchBoard::doRender(wxDC &dc)
{
	...
	/*left*/
    if (switch_left) {
        is_enable ? dc.SetBrush(wxBrush(wxColour(230, 168, 0))) : dc.SetBrush(disable_color);  // CoPrintSlicer yellow
	...
	/*right*/
    if (switch_right) {
        if (is_enable) {dc.SetBrush(wxBrush(wxColour(230, 168, 0)));  // CoPrintSlicer yellow
        } else {dc.SetBrush(disable_color);}
