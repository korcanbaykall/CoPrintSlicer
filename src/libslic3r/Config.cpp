...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Look for Slic3r or CoPrintSlicer header.
    ...
    {
        //BBS
        std::string bambuslicer_gcode_header = "; CoPrintSlicer";

        std::string coprintslicer_gcode_header = std::string("; generated by ");
        coprintslicer_gcode_header += SLIC3R_APP_NAME;
        ...
        while (std::getline(ifs, header)) {
            ...
            if (strncmp(bambuslicer_gcode_header.c_str(), line_c, strlen(bambuslicer_gcode_header.c_str())) == 0 ||
                strncmp(coprintslicer_gcode_header.c_str(), line_c, strlen(coprintslicer_gcode_header.c_str())) == 0) {
                header_found = true;
                break;
            }
        }
